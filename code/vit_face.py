...
class Softmax(nn.Module):
    ...
    def forward(self, input, label):
        if self.device_id == None:
            out = F.linear(input, self.weight, self.bias)   # ← keep original behavior
        else:
            x = input
...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.heads = heads
        self.scale = dim ** -0.5   # ← keep EXACTLY as in your original code
...
class ViT_face(nn.Module):
    ...
    # ---------------- NEW: FORWARD WITH ATTENTION (for heatmaps only) -----------
    @torch.no_grad()
    def forward_with_attention(self, img, label=None, mask=None):
        """
        Runs the SAME computation as forward(), but explicitly computes attention
        weights inside this method (without touching the original modules),
        and returns them per block.

        Returns:
          if label is None: (emb, attn_list)
          else: (logits, emb, attn_list)
        where each attn in attn_list has shape [B, heads, T, T].
        """
        self.eval()
        p = self.patch_size

        # patch embedding (same as forward)
        x = rearrange(img, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = p, p2 = p)
        x = self.patch_to_embedding(x)
        b, n, d = x.shape
        ...
        attn_list = []
        # manual pass through transformer layers (replicating Residual(PreNorm(.)))
        for attn_block, ff_block in self.transformer.layers:
            # attn_block: Residual(PreNorm(Attention))
            pren = attn_block.fn        # PreNorm(Attention)
            ln   = pren.norm
            attn = pren.fn              # Attention module with to_qkv, to_out, heads, scale

            xn = ln(x)                  # LayerNorm
            # compute qkv with the module's weights
            qkv = attn.to_qkv(xn).chunk(3, dim=-1)
            h = attn.heads
            q = rearrange(qkv[0], 'b n (h d) -> b h n d', h=h)
            k = rearrange(qkv[1], 'b n (h d) -> b h n d', h=h)
            v = rearrange(qkv[2], 'b n (h d) -> b h n d', h=h)

            dots = torch.einsum('bhid,bhjd->bhij', q, k) * attn.scale  # [B,H,T,T]
            if mask is not None:
                mask_value = -torch.finfo(dots.dtype).max
                m = F.pad(mask.flatten(1), (1, 0), value=True)
                m = m[:, None, :] * m[:, :, None]
                dots.masked_fill_(~m, mask_value)

            attn_w = dots.softmax(dim=-1)                          # [B,H,T,T]
            attn_list.append(attn_w.detach())

            y = torch.einsum('bhij,bhjd->bhid', attn_w, v)
            y = rearrange(y, 'b h n d -> b n (h d)')
            y = attn.to_out(y)                                     # uses module's proj+dropout
            x = x + y                                              # residual

            # ff_block: Residual(PreNorm(FeedForward))
            pren2 = ff_block.fn
            x = x + pren2.fn(pren2.norm(x))

        # pool + head (same as forward)
        out = x.mean(dim=1) if self.pool == 'mean' else x[:, 0]
        out = self.to_latent(out)
        emb = self.mlp_head(out)

        if label is not None:
            logits = self.loss(emb, label)
            return logits, emb, attn_list
        else:
